...
public final class ChatModificationsModule extends Module
{
    ...
    public final Value<Boolean> AntiSwearing = new Value<Boolean>("AntiSwearing", new String[] {"NoCuss"}, "Tells players off for cussing in chat.", false);
        ...
        if (p_Event.getPacket() instanceof SPacketChat)
        {
            ...
            if (packet.getChatComponent() instanceof TextComponentString)
            {
                ...
                if (component.getFormattedText().contains("> "))
                {
                    ...
                    if (swearList(l_Text.toLowerCase()) && AntiSwearing.getValue())
                        p_Event.cancel();
    ...
    private boolean swearList(String input) {
